module.exports.run = async (client, message, args) => {
    ...
    let command = client.commands.get(client.aliases.get(args.join(` `))) || client.commands.get(args.join(` `));
    if (command) {
        let field = [];
        ...
        message.channel.send(client.embed().setAuthor(`Command: ${command.code.title.substring(0, 1).toUpperCase()}${command.code.title.substring(1)}`).setDescription(field.join(`\n`)).setThumbnail(client.user.avatarURL({ format: "png", dynamic: true, size: 2048 })));
        return client.log(message);
    } else { client.src.invalid(message); return client.log(message); };
}

module.exports.run = async (client, message, args) => {
  ...
  try {
    ...
    function filter(msg) { const pattern = /(^[1-9][0-9]{0,1}$)/g; return pattern.test(msg.content) && parseInt(msg.content.match(pattern)[0]) <= 10; }
  ...
  } catch (error) { console.error(error); message.channel.activeCollector = false; };
